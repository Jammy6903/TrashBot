...
public class commandsAdmin {
  public static void adminCommands(MessageReceivedEvent event) {
    ...
    switch (args.get(0)) {
      case "set-config":
        if (args.size() == 1) {
          event.getChannel().sendMessage("**Command Usage:** a!set-config <configName>");
          return;
        }
        App.getProps().setProperty("CURRENT_CONFIG", args.get(1));
        App.CONFIG = new config(args.get(1));
        event.getMessage().reply("Config set to " + args.get(1));
        break;
      case "load-config":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!load-config <configName>");
          return;
        }
        App.CONFIG = new config(args.get(1));
        event.getMessage().reply("Config " + args.get(1) + " loaded");
        break;
      case "save-config":
        App.CONFIG.saveConfig();
        event.getMessage().reply("Config " + App.CONFIG.getConfigName() + " saved");
        break;
      case "show-config":
      ...
      case "set-config-name":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-config-name <configName>");
          return;
        }
        App.CONFIG.setConfigName(args.get(1));
        event.getMessage().reply("Config name set to " + args.get(1));
        break;
      case "set-bot-status":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-bot-status <status>");
          return;
        }
        String status = "";
        ...
        event.getMessage().reply("Status set to " + status);
        break;
      case "set-exp-increment":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-exp-increment <exp>");
          return;
        }
        try {
          App.CONFIG.setExpIncrement(Long.valueOf(args.get(1)));
          event.getMessage().reply("Exp Increment set to " + args.get(1));
        } catch (Exception e) {
          event.getMessage().reply("**Command Usage:** a!set-exp-increment <exp>");
          return;
        }
      ...
      case "set-exp-variation":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-exp-variation <variation>");
          return;
        }
        try {
          App.CONFIG.setExpVariation(Long.valueOf(args.get(1)));
          event.getMessage().reply("Exp Variation set to " + args.get(1));
        } catch (Exception e) {
          event.getMessage().reply("**Command Usage:** a!set-exp-variation <variation>");
          return;
        }
      ...
      case "set-exp-cooldown":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-exp-cooldown <cooldown>");
          return;
        }
        try {
          App.CONFIG.setExpCooldown(Long.valueOf(args.get(1)));
          event.getMessage().reply("Exp Cooldown set to " + args.get(1));
        } catch (Exception e) {
          event.getMessage().reply("**Command Usage:** a!set-exp-cooldown <cooldown>");
          return;
        }
      ...
      case "set-level-base":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-level-base <exp>");
          return;
        }
        try {
          App.CONFIG.setLevelBase(Long.valueOf(args.get(1)));
          event.getMessage().reply("Level Base set to " + args.get(1));
        } catch (Exception e) {
          event.getMessage().reply("**Command Usage:** a!set-level-base <exp>");
          return;
        }
      ...
      case "set-level-growth":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!set-level-growth <growth>");
          return;
        }
        try {
          App.CONFIG.setLevelGrowth(Long.valueOf(args.get(1)));
          event.getMessage().reply("Level Growth set to " + args.get(1));
        } catch (Exception e) {
          event.getMessage().reply("**Command Usage:** a!set-level-growth <growth>");
          return;
        }
      ...
      case "add-disabled-feature":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!add-disabled-feature <feature> [features...]");
          return;
        }
        ...
        event.getMessage().reply(addedFeatures);
        break;
      case "remove-disabled-feature":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!remove-disabled-feature <feature> [features...]");
          return;
        }
        ...
        event.getMessage().reply(removedFeatures);
        break;
      case "add-disabled-command":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!add-disabled-command <command> [commands...]");
          return;
        }
        ...
        event.getMessage().reply(addedCommands);
        break;
      case "remove-disabled-command":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!remove-disabled-command <command> [commands...]");
          return;
        }
        ...
        event.getMessage().reply(removedCommands);
        break;
      case "add-admin":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage:** a!add-admin <userId> [userIds...]");
          return;
        }
        ...
        event.getMessage().reply(addedIds);
        if (addError) {
          event.getMessage().reply(addErrorString);
        }
        break;
      case "remove-admin":
        if (args.size() == 1) {
          event.getMessage().reply("**Command Usage: a!remove-admin <userId> [userIds...]");
          return;
        }
        boolean error = false;
        ...
        event.getMessage().reply(removedIds);
        if (error) {
          event.getMessage().reply(errorString);
        }
        break;
      default:
