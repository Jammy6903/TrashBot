...
public class commandsWordCount {
  ...
  private static void infoCommand(SlashCommandInteractionEvent event) {
    ...
    if (global) {
      embed = globalWordInfo(new word(word), definition);
    } else {
  ...
  private static void leaderboardCommand(SlashCommandInteractionEvent event) {
    ...
    if (global) {
      embed = generateGlobalLeaderboard(word.getWords(10, page, order, reverse));
    } else {
      guild g = new guild(event.getGuild().getIdLong());
      String guildName = event.getGuild().getName();
  ...
  private static EmbedBuilder globalWordInfo(word word, String definition) {
  ...
  private static EmbedBuilder generateGuildLeaderboard(guild g, String guildName, List<guildWord> words) {
    String content = "";

    for (int i = 0; i < words.size(); i++) {
      guildWord w = words.get(i);
